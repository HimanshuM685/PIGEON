...
exports.findOnboardedUser = findOnboardedUser;
exports.insertOnboardedUser = insertOnboardedUser;
exports.findUserByTelegramId = findUserByTelegramId;
exports.insertTelegramUser = insertTelegramUser;
exports.linkTelegramToPhone = linkTelegramToPhone;
require("dotenv/config");
const algokit_utils_1 = require("@algorandfoundation/algokit-utils");
const ContractPigeonClient_1 = require("./contract/ContractPigeonClient");
...
function getAlgorand() {
    if (!_algorand) {
        _algorand = algokit_utils_1.AlgorandClient.fromConfig({
            algodConfig: {
                server: ALGOD_SERVER,
                token: ALGOD_TOKEN,
...
function getAdminAccount() {
    if (!ADMIN_MNEMONIC) {
        throw new Error("ADMIN_MNEMONIC is not set — cannot interact with the on-chain contract");
    }
...
function getAppClient() {
    if (!_appClient) {
        if (!PIGEON_APP_ID || PIGEON_APP_ID === 0n) {
            throw new Error("PIGEON_APP_ID is not set — deploy the ContractPigeon first and set the app ID");
        }
        const admin = getAdminAccount();
        _appClient = new ContractPigeonClient_1.ContractPigeonClient({
 ...
 */
async function findOnboardedUser(phone) {
 ...
 */
async function insertOnboardedUser(phone, address, encryptedMnemonic) {
    const normalised = normalizePhone(phone);
    const client = getAppClient();
 ...
 */
async function insertTelegramUser(telegramId, address, encryptedMnemonic, telegramHandle) {
    const client = getAppClient();
    const tgId = telegramId.toString();
 ...
 */
async function linkTelegramToPhone(telegramId, phone, telegramHandle) {
    const normalised = normalizePhone(phone);
    const client = getAppClient();
    const tgId = telegramId.toString();
