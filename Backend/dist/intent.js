...
function parseIntentJson(text) {
    ...
    if (["send", "get_balance", "get_txn", "onboard", "get_address", "fund", "get_pvt_key", "unknown"].includes(t))
        return t;
    return "unknown";
}
...
async function getIntent(message, apiKey) {
    const openrouter = new sdk_1.OpenRouter({ apiKey });
