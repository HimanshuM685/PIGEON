...
exports.getIntentLocal = getIntentLocal;
// ─── Command Patterns ───────────────────────────────────────────────────────
...
const SEND_PATTERN = /^(?:send|transfer)\s+(\d+(?:\.\d+)?)\s*(?:algo)?\s*(?:to)\s+(\S+)(?:\s+(?:password|pass|pw|pin)\s+(.+))?$/i;
// Balance
const BALANCE_PATTERN = /^(?:balance|get\s+balance|bal|check\s+balance|how\s+much)$/i;
// Address
const ADDRESS_PATTERN = /^(?:address|get\s+address|addr|my\s+address|show\s+address|show\s+my\s+address)$/i;
// Fund
const FUND_PATTERN = /^(?:fund\s*me|fund|get\s+fund|fund\s+my\s+wallet)$/i;
// Transactions
const TXN_PATTERN = /^(?:get\s+txn|txn|transactions|history|last\s+transaction|txn\s+status|get\s+transactions)$/i;
// Create wallet / onboard (without mnemonic)
const CREATE_WALLET_PATTERN = /^(?:create\s+wallet|create\s+account|sign\s*up|onboard|register)(?:\s+(?:password|pass|pw|pin)\s+(.+))?$/i;
// Import wallet (with mnemonic)
const IMPORT_WALLET_PATTERN = /^import\s+wallet\s+(.+?)(?:\s+(?:password|pass|pw|pin)\s+(.+))?$/i;
// Get private key / recovery phrase
const PVT_KEY_PATTERN = /^(?:get\s+pvt\s+key|private\s+key|export\s+key|recovery\s+phrase|seed\s+phrase|mnemonic|get\s+private\s+key|show\s+private\s+key)(?:\s+(?:password|pass|pw|pin)\s+(.+))?$/i;
// ─── Classifier ─────────────────────────────────────────────────────────────
/**
 ...
 */
function getIntentLocal(message) {
    ...
    if (pvtMatch) {
        const params = {};
        if (pvtMatch[1])
            params.password = pvtMatch[1].trim();
        return {
            intent: 'get_pvt_key',
            params,
            rawMessage: message,
    ...
    if (importMatch) {
        ...
        if (importMatch[2])
            params.password = importMatch[2].trim();
        return {
            intent: 'onboard',
            params,
            rawMessage: message,
    ...
    if (createMatch) {
        const params = {};
        if (createMatch[1])
            params.password = createMatch[1].trim();
        return {
            intent: 'onboard',
            params,
            rawMessage: message,
    ...
    if (sendMatch) {
        ...
        if (sendMatch[3])
            params.password = sendMatch[3].trim();
        return {
            intent: 'send',
            params,
            rawMessage: message,
